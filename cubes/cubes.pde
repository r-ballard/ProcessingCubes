  ...
  if (line == null) {
    ...
    while (videoExport.getCurrentTime() < soundTime + frameDuration * 0.5) {
      ...
      for (int i = 1; i < nbCubes; i++)
      {
        ...
        print("scoreMid: ",scoreMid,"\n");
        print("scoreHi: ",scoreHi,"\n");
        print("bandValue: ",bandValue,"\n");
        print("scoreGlobal: ",scoreGlobal,"\n");
      ...
      for (int i = 1; i < p.length; i++)
      {
        ...
        float bandValue = float(p[i])*(1 + (i/50));
      ...
      for (int i = 0; i < nbMurs; i++)
      {
        ...
        float intensity = float(p[i%((int)(p.length*specHi))]);
